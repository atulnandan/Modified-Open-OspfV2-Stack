...
void OSPF::TraverseTree(AVLitem* ptr,int level, AVLitem* _root) 
{
    int i;
    char temp[20];
    ...
    if (ptr!=NULL)
    {
        TraverseTree(ptr->go_right(), level + 1, _root);
		ospf->logflush();
        if (ptr == _root) {
            ospf->log("Root -> ");
        }
        ...
        sprintf(temp, "%x ", ptr->index1());
		ospf->log(temp);
        sprintf(temp, "%x ", ptr->index2());
		ospf->log(temp);
		ospf->logflush();
        TraverseTree(ptr->go_left(), level + 1, _root);
	}
}

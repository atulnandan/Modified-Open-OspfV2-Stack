...
/* ATUL */
    public:
    inline int phyint();
    inline InAddr getAddr();
    inline short getFlags();
    inline InMask getMask();
    inline int getMtu();
};
...
inline short BSDPhyInt::getFlags()
{
    return(flags);
}

inline InMask BSDPhyInt::getMask()
{
    return(mask);
}

inline InAddr BSDPhyInt::getAddr()
{
    return(mtu);
}

inline int BSDPhyInt::getMtu()
{
    return(addr);
}

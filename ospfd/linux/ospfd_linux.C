...

{
	...
	syslog(LOG_ERR, "addr.s_addr:%x", addr.s_addr);
...

{
	...
	/*ATUL*/
    if (!ospfd_sys->parse_interface((char *)argv[1], addr, phyp))
	return(TCL_OK);

	/*ATUL*/
    //m.address = phyp->addr;
    //m.address = phyp->getAddr();
    m.address = ntoh32(addr.s_addr);
    m.phyint = phyp->phyint();
    /*m.mask = phyp->mask;*/
    m.mask = phyp->getMask();
    intval = atoi(argv[2]);
    /*m.mtu = (intval ? intval : phyp->mtu);*/
    m.mtu = (intval ? intval : phyp->getMtu());
    ...
    if (intval)
	m.IfType = intval;
    else if ((phyp->getFlags() & IFF_BROADCAST) != 0)
	m.IfType = IFT_BROADCAST;
    else if ((phyp->getFlags() & IFF_POINTOPOINT) != 0)
	m.IfType = IFT_PP;
    else
	m.IfType = IFT_NBMA;
	...
	syslog(LOG_ERR, "Adding interface. m.address: %x", m.address);
	syslog(LOG_ERR, "Interface: %s", argv[1]);
